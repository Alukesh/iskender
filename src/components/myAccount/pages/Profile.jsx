import React, { useRef, useState, useEffect } from 'react';
import { useForm } from 'react-hook-form';
import * as Icons from '../../icons/myAccount';
import kyrgyzstan_icon from '../../icons/Kyrgyzstan.svg';
import arrowDown_icon from '../../icons/arrow-down.svg';
import userMy_icon from '../../icons/User.svg';
import {ReactComponent as NotificationIcon} from '../../icons/notification.svg';
import messageMy_icon from '../../icons/Message.svg';
import locationMy_icon from '../../icons/Location-i.svg';
import calendarMy_icon from '../../icons/calendar.svg';
import { useDispatch, useSelector } from 'react-redux';
import ReturnUrlFromIp from '../../../hooks/useReplaceUrl';
import api, { updateAvatar } from '../../../services/api';
import { Toast } from 'primereact/toast';

const Profile = () => {
  const { user } = useSelector(store => store.cart)
  const dispatch = useDispatch()
  const toast = useRef(null);
  const showToast = (msg) => {
    toast.current.show({
      severity: 'info',
      detail: msg,
      className: 'toaster Gilroy-m',
      icon: <NotificationIcon style={{marginRight:'36px'}}/>
    });
  };

  const inputFileRef = useRef(null)
  const [deployImage, setDeployImage] = useState('data:image/jpeg;base64,')
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm({
    mode: 'onChange',
  });


  const [getBirthDate, setGetBirthDate] = useState('');
  const [formChanged, setFormChanged] = useState(false);
  const [selectedGender, setSelectedGender] = useState(user?.gender);
  const handleChange = () => {
    // setFormChanged(true);
  };


  const onSubmit = (data) => {
    const formData = { ...data, gender: selectedGender };
    console.log(formData, 1);
    api.post('/client/updateClientInfo', { clientId: user?._id, userInfo: formData })
      .then(({ data }) => {
        showToast('Профиль сохранен')
        dispatch.cart.afterLoginUser({ ...user, ...formData, birthDate: formData.dateofBirth })
        localStorage.setItem('isk user', JSON.stringify({ ...user, ...formData, birthDate: formData.dateofBirth }))
      })
      .catch(err => console.log(err))
    setFormChanged(false);
  };
  /////////////////
  const handleSelectImage = (e) => {
    if (!e.target.files.length) return;
    const file = e.dataTransfer.files[0]
    // const formData = new FormData()
    // formData.append('clientId', user?._id)
    // formData.append('file_name', file)
    updateAvatar({ 'clientId': user?._id, 'fileName': file?.name })
      .then(({ data }) => {
        showToast('Данные успешно изменились')
        console.log(1);
        setDeployImage(data.avatarUrl);
        dispatch.cart.afterLoginUser({ ...user, avatar: data.name })
      })
      .catch(err => console.log(err))
  }

  const handleGenderButtonClick = (gender) => {
    setSelectedGender(gender);
  };


  useEffect(() => {
    if (user && user.birthDate) {
      setGetBirthDate(user.birthDate.slice(0, 10));
    }
    if (user && user.gender) {
      setSelectedGender(user.gender);
    }
  }, [user]);


  return (
    <div className='personal_data'>
      <h1 className='personal_data_title Gilroy-w'>Личные данные</h1>
      <div className='top_block'>
        <div className='left'>
          <div className='avatar_update'>
            <img className='avatar' src={ReturnUrlFromIp(user?.avatar) || deployImage} alt='your photo' />
            <input type="file" ref={inputFileRef} accept="image/png, image/jpeg, image/webp, image/jpg" onChange={handleSelectImage} className='inputAvatar' />
            <button onClick={() => inputFileRef.current.click()} className='upload_button'>Загрузить фото</button>
          </div>
          <div className='bonus_indebtedness'>
            <div className='block'>
              <h1 className='title'>Бонусы</h1>
              <span className='line'></span>
              <p className='som'>{user?.points} c</p>
            </div>
            <div className='block'>
              <h1 className='title'>Дебиторская задолженность</h1>
              <span className='line'></span>
              <p className='som'>{user?.balance < 0 ? `-${user?.balance}` : user?.balance} c</p>
            </div>
          </div>
          <img className='qr_code' src={Icons.QrCodeIcon} alt='QR code' />
        </div>
        {/* ! form */}
        <div className='right'>
          <form className='form' onSubmit={handleSubmit(onSubmit)}>
            <div className='input_high_block'>
              <label htmlFor='user_name'>ФИО</label>
              <div className='input_block'>
                <img className='icon' src={userMy_icon} alt='' />
                <input
                  type='text'
                  className={`input ${errors.name && 'error'}`}
                  placeholder='ФИО'
                  name='name'
                  // onChange={handleChange}
                  defaultValue={user?.name}
                  {...(user?.name ? register('name', { required: true, minLength: 2 }) : {})}
                />
              </div>
            </div>
            <div className='input_high_block'>
              <label htmlFor='user_name'>Номер телефона</label>
              <div className='input_block'>
                <div className='input input_wrap'>
                  <div className='input_wrap__flex'>
                    <img className='icon' src={kyrgyzstan_icon} alt='' />
                    <img className='icon' src={arrowDown_icon} alt='' />
                  </div>

                  <input
                    type='text'
                    className={`input input_wrap__phone ${errors.phone && `error`}`}
                    placeholder='Номер телефона'
                    // onChange={handleChange}
                    defaultValue={user?.phone}
                    {...(user?.phone ? register('phone', { required: true }) : {})}
                  />
                </div>
              </div>
            </div>
            <div className='input_high_block'>
              <label htmlFor='user_name'>E- mail</label>
              <div className='input_block'>
                <img className='icon' src={messageMy_icon} alt='' />
                <input
                  type='text'
                  className={`input ${errors.email && `error`}`}
                  onChange={() => {}}
                  placeholder='E- mail'
                  // onChange={handleChange}
                  defaultValue={user?.email}
                  {...( register('email', {
                    required: true,
                    pattern: /^\S+@\S+$/i,
                  }) )}
                />
              </div>
            </div>
            <div className='input_high_block'>
              <label htmlFor='user_name'>Дата рождения</label>
              <div className='input_block'>
                <img className='icon' src={calendarMy_icon} alt='' />
                <input
                  type='date'
                  className={`input input_date ${errors.date && `error`}`}
                  // onChange={handleChange}
                  defaultValue={getBirthDate}
                  {...( register('dateofBirth', { required: true }) )}
                />
              </div>
            </div>
            <div className='input_high_block'>
              <label htmlFor='user_name'>Город</label>
              <div className='input_block'>
                <img className='icon' src={locationMy_icon} alt='' />
                <input
                  type='text'
                  className={`input ${errors.city && `error`}`}
                  placeholder='Город'
                  // onChange={handleChange}
                  defaultValue={user?.city}
                  {...(user?.city ? register('city', { required: true, minLength: 2 }) : {})}
                />
              </div>
            </div>
            <div className='input_high_block'>
              <label htmlFor='user_name'>Пол</label>
              <div className='buttons_block'>
                <span
                  className={`button_sex ${selectedGender === 'Male' ? 'active' : ''}`}
                  onClick={() => handleGenderButtonClick('Male')}
                >
                  Мужчина
                </span>
                <span
                  className={`button_sex ${selectedGender === 'Female' ? 'active' : ''}`}
                  onClick={() => handleGenderButtonClick('Female')}
                >
                  Женщина
                </span>
              </div>
            </div>
            <span className='line'></span>
            <div className='input_high_block'>
              <div className='buttons_block'>
                <button type='submit' disabled={formChanged} className='button_submit active'>
                  Сохранить
                </button>
                <span className='button_submit undo'>Отмена</span>
              </div>
            </div>
          </form>
        </div>
      </div>

      <Toast
        ref={toast}
      />
    </div>
  );
};

export default Profile;
